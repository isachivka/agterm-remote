 ...
 * parent. That works while every screen has exactly one way in, and REQ-0004 ends that: the Updates
 * screen is reachable from the launcher's tile and from the update banner, and a hardcoded parent
 * cannot be right for both. Back stops being a fact about a screen and becomes a fact about the
 * journey, so it is stored as one.
 *
...
data class BackStack(val entries: List<Screen>) {
     ...
     *    Without this, Home → Updates → Token → Updates would put back at Token, sending the owner
     *    forwards through a screen they just left. It also keeps the stack bounded by the number of
    ...
    companion object {
        val Initial = BackStack(listOf(Screen.Home))
         ...
         * came from a different version of the app — which is what happens when the owner updates
         * while the app is in the background, and this app updates itself.
         */
        ...
        val StateSaver: Saver<BackStack, Any> = listSaver<BackStack, String>(
            ...
            restore = { routes ->
                BackStack(routes.mapNotNull(Screen::fromRoute).ifEmpty { listOf(Screen.Home) })
            },
        )
    }
}
