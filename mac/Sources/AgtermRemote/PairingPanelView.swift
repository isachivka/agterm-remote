...
/// **The pairing panel, as a view that two windows draw.**
///
/// It was the body of `PairingWindow` alone. The last pane of setup then had to send somebody to that
/// window to see a code - a heading promising a code, a button, and a second window - and the first
/// person to set this up asked why the code was not simply there. It is, now: the setup window draws
/// this view on its last pane, and the menu's *Pair a phone…* draws the same view in its own window.
/// One rendering of one payload, so the two cannot drift.
///
/// **Every state renders something.** A view that is blank is a mystery, not an affordance — the same
/// rule that cost the owner a pairing on 2026-08-10 when a menu item looked pressable and was not.
///
