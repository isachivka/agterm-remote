...
struct BridgeProcessTests {
    ...
    /// Launches nothing. Records what it was asked for, and hands the exit back when told to.
    private final class RecordingLauncher: ProcessLauncher, @unchecked Sendable {
        ...
        func launch(
            ...
            lock.withLock {
                ...
                _launches += 1
                _lastPid = 4242 + Int32(_launches)
                onExitByPid[_lastPid] = onExit
                return _lastPid
            }
        }

        /// A real `Process` reports the termination it was asked for, so this one does too: a fake
        /// that terminated silently would let a state machine that never handles the exit pass.
