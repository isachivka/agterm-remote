...
struct AddressPaneTests {
    ...
    @Test func theWindowRendersTheCopyRatherThanRepeatingIt() throws {
        ...
        for reference in ["OnboardingCopy.addressPlaceholder"] {
...
struct EditingMenuTests {
    ...
    @Test func theAppSuppliesItFromTheStore() throws {
        ...
        guard let main else { return #expect(Bool(false), "main.swift is not in the walk") }
        #expect(
            main.code.contains("isPaired: AddressPreference.provenAt() != nil"),
            "the setup ladder is built without the pairing fact, so the code pane is unreachable")
        // **And nothing asks how the phone reaches this Mac.** The question was removed in favour of
        // the bridge answering both dresses on one port and the phone trying TLS first; a popup that
        // came back would be the two-days-two-wrong-answers defect returning.
        #expect(
            !main.code.contains("frontDoor") && !main.code.contains("FrontDoor"),
            "the front-door question is back in main.swift")
    }
}
