...
func (r *refusalRate) record() {
	...
	if r.windowEnd.IsZero() {
		r.windowEnd = now.Add(refusalReportEvery)
		return
	}
...
func causeText(err error) string {
	...
	// A failure of this handler's own rather than the window's - these carry their own words, written
	// here and never derived from the caller's bytes.
	return err.Error()
}
